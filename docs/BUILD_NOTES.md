...
### Keyboard-friendly booking panel
- Replace the cramped in-scene form with a booking invitation and a dedicated native modal outside the animated world. Mobile form entry uses the full visible viewport; desktop gets a centered panel. The fixed header/close button and one vertically scrolling form replace the small scroll strip above the characters.
- Size and position the panel from VisualViewport resize/scroll events so the software keyboard leaves usable space. Use single-column 16px inputs with 50px controls, a 120px message area, and keep focused fields within the visible scrolling region.
- Lock the background for the whole form session instead of individual field focus. Preserve the journey camera through opening, keyboard/orientation resizing, and closing; restore trigger focus without discarding the draft. The form remains mounted between visits and the existing server submission behavior is unchanged.
- Verified 390×667 mobile entry, typed input at a simulated 390×367 keyboard viewport, field navigation, reachable submit button, close/reopen draft retention, desktop presentation, Escape/focus return, and return to chapter 4. No inquiry was submitted during UI checks. Actual iPhone keyboard behavior remains a physical-device follow-up. Build, targeted ESLint, and all eight mocked booking integration tests pass.


### Keep band scale stable when mobile browser chrome changes
