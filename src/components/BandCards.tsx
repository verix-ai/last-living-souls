  ...
  return <>
    ...
    <div className="band-card-track">
      {bandMembers.map((member, i) => <article className={`member-card card-${member.id}`} key={member.id} style={{ '--float-delay': `${i * -.8}s` } as CSSProperties}>
        <div className="member-portrait"><img src={`/art/band/heads/${member.id}.png${member.id === 'daniel' ? '?v=western-face-3' : ''}`} width="96" height="96" alt={`${member.name}'s pixel-art portrait`} /></div>
