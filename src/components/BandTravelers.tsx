...
export default function BandTravelers() {
  ...
  return <div className="band-ensemble">
    ...
    {members.map(member => <li className={`band-member member-${member.id}`} key={member.id}
      ...
      <span className="band-sprite" aria-hidden="true"><img className="band-sprite-sheet" src={`/art/band/${member.id}.png`} width="480" height="160" alt="" loading="eager" decoding="sync" fetchPriority="high" draggable={false} /></span>
      {['daniel', 'drew'].includes(member.id) && <img className="performance-sprite" src={`/art/band/${member.id}-performance.png`} width="160" height="160" alt="" aria-hidden="true" loading="eager" onLoad={event => { void preparePerformance(member.id, event) }} onError={() => setPerformanceReady(ready => ({ ...ready, [member.id]: false }))} />}
