...
export default function App() {
  ...
  useEffect(() => {
    const visitHash = () => { const aliases: Record<string, string> = { forest: 'band', transmission: 'wasteland', sanctuary: 'portal' }; const hash = location.hash.slice(1); const index = chapters.findIndex(c => c.id === (aliases[hash] || hash)); if (index >= 0) requestAnimationFrame(() => goTo(index, true)) }
    visitHash()
