...
export default function App() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      world.current?.style.setProperty('--drift', `${Math.sin(progress * Math.PI) * 24}px`)
      world.current?.style.setProperty('--turn', `${Math.sin(progress * 1.5) * 8}deg`)
  ...
  const collect = (index: number) => {
    ...
    announce(next.length === 3 ? 'YOUR LIGHT FOUND A FELLOW TRAVELER.' : `YOUR LIGHT: ${next.length} / 3 STARS FOUND.`)
  }
  ...
  return <div className={`experience ${calm ? 'calm' : ''}`}>
    ...
    <main ref={journey} className="journey" aria-label="The Last Signal, a journey with Last Living Souls"><div ref={world} className="world" data-scene={active}><div className="world-track">
      <section className="scene scene-wasteland" id="wasteland" aria-labelledby="title-wasteland" inert={!calm && active !== 0}><img className="scene-art" src="/art/wasteland.webp" alt="" fetchPriority="high" /><div className="scene-shade" /><div className="scene-content opening-content"><div className="eyebrow"><span className="tiny-cross">✦</span> A PSYCHEDELIC EXPEDITION <span className="tiny-cross">✦</span></div><h1 id="title-wasteland"><span className="sr-only">Last Living Souls</span><img className="hero-wordmark" src="/art/wordmark.png" alt="" width="800" height="289" /></h1><p className="opening-line">Somewhere between the end of the world<br className="desktop-break" /> and the start of a song.</p><button className="pixel-button journey-start" onClick={() => goTo(1)}>Follow the signal <PixelIcon name="arrow" /></button><span className="scroll-instruction">SCROLL TO WANDER <span>↓</span></span></div><div className="scene-caption"><span>01 / THE WASTELAND</span><span>THERE’S SOMETHING OUT THERE.</span></div>{signal(0)}</section>
      <section className="scene scene-forest" id="forest" aria-labelledby="title-forest" inert={!calm && active !== 1}><img className="scene-art" src="/art/spore-forest.webp" alt="" /><div className="scene-shade" /><div className="scene-content forest-content"><div className="eyebrow">02 / THE SPORE FOREST</div><h2 id="title-forest">Still here.<br /><em>Still wandering.</em></h2><p>Last Living Souls.<br />Psychedelic rock for the long way home.</p><ExternalLink href={links.instagram} className="text-link">Find the souls <span>↗</span></ExternalLink></div><div className="scene-caption"><span>FOLLOW THE GLOW</span><span>LIFE FINDS A WAY.</span></div>{signal(1)}</section>
    ...
    </div><BandTravelers />{signals.length === 3 && <div className="traveler" aria-hidden="true"><div className="traveler-arrival"><div className="traveler-float"><img src="/art/astronaut.png" alt="" width="444" height="426" /><div className="traveler-shadow" /></div></div></div>}<div className="scanlines" aria-hidden="true" /></div></main>
