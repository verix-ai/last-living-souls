...
const links = {
  ...
  track: 'https://open.spotify.com/track/6h5yX5qxLx8IW2afIuMXt7',
...
const chapters = [
  { id: 'wasteland', name: 'The Wasteland', note: 'Every ending has a frequency.' },
  { id: 'forest', name: 'The Spore Forest', note: 'Something is still alive out here.' },
  { id: 'transmission', name: 'The Transmission', note: 'A little closer to the source.' },
  { id: 'sanctuary', name: 'The Living Stage', note: 'Let the sound take over.' },
  { id: 'portal', name: 'The Portal', note: 'The journey doesn’t end here.' },
]
type IconName = 'spotify' | 'apple' | 'instagram' | 'facebook' | 'tiktok' | 'play' | 'star' | 'arrow' | 'map' | 'pause'
...
export default function App() {
  ...
  const goTo = useCallback((index: number, instant = false) => {
    ...
    const top = calm ? (scene?.offsetTop ?? 0) : root.offsetTop + ((root.offsetHeight - window.innerHeight) * bounded / 4)
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const root = journey.current
      if (!root) return
      let progress = Math.max(0, Math.min(4, (window.scrollY - root.offsetTop) / Math.max(1, root.offsetHeight - window.innerHeight) * 4))
      if (calm) { progress = [...root.querySelectorAll<HTMLElement>('.scene')].reduce((selected, element, i) => element.getBoundingClientRect().top <= window.innerHeight * .5 ? i : selected, 0) }
      const index = Math.round(progress)
      progressRef.current = progress
      if (activeRef.current !== index) { activeRef.current = index; setActive(index) }
      world.current?.style.setProperty('--travel', `${-progress * 100}vw`)
      ...
      if (bar.current) bar.current.style.transform = `scaleX(${progress / 4})`
    }
    const onScroll = () => {
  ...
  const signal = (index: number) => <button className={`lost-signal signal-${index} ${signals.includes(index) ? 'collected' : ''} ${hint ? 'hinted' : ''}`} onClick={() => collect(index)} aria-label={signals.includes(index) ? 'Star found' : `Collect the hidden star in ${chapters[index].name}`} disabled={signals.includes(index)}><PixelIcon name="star" /><span>{signals.includes(index) ? 'FOUND' : 'YOUR LIGHT'}</span></button>
  return <div className={`experience ${calm ? 'calm' : ''}`}>
    <a href="#transmission" className="skip-link" onClick={e => { e.preventDefault(); goTo(2, true); setTimeout(() => document.querySelector<HTMLAnchorElement>('#transmission .platform')?.focus(), 100) }}>Skip to music</a>
    <header className="hud-top"><button className="home-mark" onClick={() => goTo(0)} aria-label="Last Living Souls, return to the beginning"><img src="/art/wordmark.png" alt="Last Living Souls" /></button><div className="coordinates"><span className="signal-light" /> LOST IN SPACE. FOUND IN SOUND.</div><div className="top-actions"><button className="listen-shortcut" onClick={() => goTo(2)}><PixelIcon name="play" /><span>Listen</span></button><button className={`map-toggle ${mapOpen ? 'selected' : ''}`} ref={mapButton} onClick={() => setMapOpen(!mapOpen)} aria-label={mapOpen ? 'Close world map' : 'Open world map'} aria-expanded={mapOpen} aria-controls="world-map"><PixelIcon name="map" /><span>{mapOpen ? 'Close' : 'World map'}</span></button></div></header>
    ...
    <main ref={journey} className="journey" aria-label="The Last Signal, a journey with Last Living Souls"><div ref={world} className="world" data-scene={active}><ContinuousWorld /><div className="world-track">
      <section className="scene scene-wasteland" id="wasteland" aria-labelledby="title-wasteland" inert={!calm && active !== 0}><div className="scene-content opening-content"><div className="eyebrow"><span className="tiny-cross">✦</span> A PSYCHEDELIC EXPEDITION <span className="tiny-cross">✦</span></div><h1 id="title-wasteland"><span className="sr-only">Last Living Souls</span><img className="hero-wordmark" src="/art/wordmark.png" alt="" width="800" height="289" /></h1><p className="opening-line">Somewhere between the end of the world<br className="desktop-break" /> and the start of a song.</p><button className="pixel-button journey-start" onClick={() => goTo(1)}>Follow the signal <PixelIcon name="arrow" /></button><span className="scroll-instruction">SCROLL TO WANDER <span>↓</span></span></div><div className="scene-caption"><span>01 / THE WASTELAND</span><span>THERE’S SOMETHING OUT THERE.</span></div>{signal(0)}</section>
      <section className="scene scene-forest" id="forest" aria-labelledby="title-forest" inert={!calm && active !== 1}><div className="scene-content forest-content"><div className="eyebrow">02 / THE SPORE FOREST</div><h2 id="title-forest">Still here.<br /><em>Still wandering.</em></h2><p>Last Living Souls.<br />Psychedelic rock for the long way home.</p><ExternalLink href={links.instagram} className="text-link">Find the souls <span>↗</span></ExternalLink></div><div className="scene-caption"><span>FOLLOW THE GLOW</span><span>LIFE FINDS A WAY.</span></div>{signal(1)}</section>
      <section className="scene scene-transmission" id="transmission" aria-labelledby="title-transmission" inert={!calm && active !== 2}><div className="scene-content transmission-content"><div className="eyebrow"><span className="signal-light" /> SIGNAL ACQUIRED</div><h2 id="title-transmission">Are you<br /><em>receiving us?</em></h2><p>Pick a frequency. Let it take you somewhere.</p><MusicLinks /><div className="frequency"><span /> LAST LIVING SOULS <span /></div></div><div className="scene-caption"><span>03 / THE TRANSMISSION</span><span>TURN ON. TUNE IN. DRIFT OUT.</span></div></section>
      <section className="scene scene-sanctuary" id="sanctuary" aria-labelledby="title-sanctuary" inert={!calm && active !== 3}><div className="scene-content sanctuary-content"><div className="eyebrow">04 / THE LIVING STAGE</div><div className="release-layout"><div className="record-art"><img src="/art/shouldve-known.jpg" alt="Should’ve Known, single artwork" width="300" height="300" loading="lazy" /><span className="record-tag">A TRANSMISSION FROM EARTH</span></div><div className="release-copy"><span className="release-label">LAST LIVING SOULS / SINGLE</span><h2 id="title-sanctuary">Should’ve<br /><em>Known.</em></h2><p>For wherever your mind wanders next.</p><button className="pixel-button" onClick={() => setPlayerOpen(!playerOpen)} aria-expanded={playerOpen} aria-controls="spotify-player"><PixelIcon name={playerOpen ? 'pause' : 'play'} />{playerOpen ? 'Close player' : 'Listen to the single'}</button><ExternalLink href={links.track} className="small-link">Open in Spotify ↗</ExternalLink></div></div>{playerOpen && <div className="embedded-player" id="spotify-player"><iframe title="Listen to Should’ve Known by Last Living Souls on Spotify" src="https://open.spotify.com/embed/track/6h5yX5qxLx8IW2afIuMXt7?theme=0" width="100%" height="152" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" /></div>}</div><div className="scene-caption"><span>YOU FOUND THE SOUND</span><span>TAKE A LITTLE OF IT WITH YOU.</span></div>{signal(3)}</section>
      <section className="scene scene-portal" id="portal" aria-labelledby="title-portal" inert={!calm && active !== 4}><div className="scene-content portal-content"><div className="eyebrow">05 / THE PORTAL</div><h2 id="title-portal">The end is<br /><em>another beginning.</em></h2><SongPreview active={active === 4} /><div className="social-links">{(['instagram', 'tiktok', 'facebook'] as const).map(name => <ExternalLink key={name} href={links[name]} className={`social-key ${name}`} label={`Last Living Souls on ${name} (opens in a new tab)`}><PixelIcon name={name} /><span>{name === 'tiktok' ? 'TikTok' : name[0].toUpperCase() + name.slice(1)}</span></ExternalLink>)}</div><div className={`completion ${signals.length === 3 ? 'complete' : ''}`}><PixelIcon name="star" /><span>{signals.length === 3 ? 'YOUR LIGHT FOUND US.' : `YOUR LIGHT: ${signals.length} / 3 STARS FOUND`}</span></div><button className="text-link" onClick={() => goTo(0)}>Wander again <span>↶</span></button></div><div className="scene-caption"><span>LAST LIVING SOULS © {new Date().getFullYear()}</span><span>THANKS FOR GETTING LOST WITH US.</span></div></section>
