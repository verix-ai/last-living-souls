...
export default function App() {
  ...
  const collect = (index: number) => {
    ...
    announce(next.length === 3 ? 'ALL SIGNALS FOUND. YOU’RE ONE OF US.' : `SIGNAL ${next.length} / 3 FOUND. KEEP WANDERING.`)
  }
  ...
  const signal = (index: number) => <button className={`lost-signal signal-${index} ${signals.includes(index) ? 'collected' : ''} ${hint ? 'hinted' : ''}`} onClick={() => collect(index)} aria-label={signals.includes(index) ? 'Signal found' : `Collect the hidden signal in ${chapters[index].name}`} disabled={signals.includes(index)}><PixelIcon name="star" /><span>{signals.includes(index) ? 'FOUND' : 'A LOST SIGNAL'}</span></button>
